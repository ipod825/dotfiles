...
function M.highlight(group, def)
    ...
    vim.api.nvim_command(string.format(
                             'highlight %s gui=%s guifg=%s guibg=%s %s', group,
                             style, fg, bg, sp))
end
...
M.syntax = {
    ...
    DiffChange = {fg = color.black, bg = color.yellow},
    DiffDelete = {fg = color.black, bg = color.red},
    DiffText = {fg = color.black, bg = color.fg},
...
for group, colors in pairs(M.syntax) do M.highlight(group, colors) end
